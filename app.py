...
DEFAULT_DB = Path('/tmp/nxn_audit.db') if os.getenv('VERCEL') else BASE_DIR / 'nxn_audit.db'
DB_PATH = Path(os.getenv('NXN_DB_PATH', str(DEFAULT_DB)))
...
def dashboard():
    ...
    with db() as c:
        ...
        audits = c.execute('SELECT a.*,b.name_ar,b.name_en FROM audits a JOIN branches b ON b.id=a.branch_id' + where + ' ORDER BY a.id DESC', params).fetchall()
        if user['role'] == 'branch' and user.get('branch_id'):
    ...
    return jsonify({'user': user, 'kpis': {'branches': len(branches), 'audits': len(audits), 'average_score': round(sum(scores) / len(scores), 1) if scores else 0, 'open_audits': sum(1 for r in audits if r['status'] in {'draft', 'submitted'})}, 'branches': [dict(r) for r in branches], 'audits': [dict(r) for r in audits]})
